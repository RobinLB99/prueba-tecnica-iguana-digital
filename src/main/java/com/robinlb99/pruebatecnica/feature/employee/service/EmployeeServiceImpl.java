...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public EmployeeServiceImpl(EmployeeRepository employeeRepository, EmployeeMapper employeeMapper, EmployeeDAOImpl employeeDAO) {
	...
	public Employee getEmployee(Integer id) {
		return employeeRepository.findById(id).orElseThrow(
				() -> new EmployeeNotFoundException("El empleado con ID '" + id + "' no ha sido encontrado."));
	}

	@Override
	@Transactional
	public Employee updateEmployee(EmployeeRequestDTO employeeRequestDTO, Integer id) {
		if (!employeeRepository.existsById(id)) {
			throw new EmployeeNotFoundException(
					"Empleado con ID '" + id + "' no existe. No se puede actualizar un Empleado inexistente.");
		}
		Employee employee = employeeMapper.toEmployee(employeeRequestDTO);
		employee.setId(id);
		return employeeRepository.save(employee);
	...
	public void transferSalary(Integer employeeIDOne, Integer employeeIDTwo, Double value) {
		if (!employeeRepository.existsById(employeeIDOne)) {
			throw new EmployeeNotFoundException("El empleado con ID '" + employeeIDOne + "' no existe");
		}
		if (!employeeRepository.existsById(employeeIDTwo)) {
			throw new EmployeeNotFoundException("El empleado con ID '" + employeeIDTwo + "' no existe");
		}
